...
public class AuditAspect {
    ...
    public Object logWithdrawal(ProceedingJoinPoint thisJoinPoint) throws Throwable
    {
        ...
        try
        {
            ...
            applicationContext.publishEvent(new WithdrawEvent(getAccountId(methodArgs), (double) methodArgs[1], WithdrawEvent.State.SUCCESSFUL));
        }
        catch (Exception e)
        {
            applicationContext.publishEvent(new WithdrawEvent(getAccountId(methodArgs), (double) methodArgs[1], WithdrawEvent.State.FAILED));
